...
interface StravaActivityPanelProps {
  /** Self-reported FTP from Strava, in watts. Null when not set. */
  ftp: number | null
  /** Per-sport-type 14-day stats. Empty when no recent activity. */
  stats: StravaSportStat[]
  /**
   * When true, hides the panel entirely if there are no stats and no FTP.
   * Useful in discovery so we don't render an empty card for users who
   * haven't connected Strava. Defaults to false (own profile context).
   */
...
export function StravaActivityPanel({
  ...
  if (hideWhenEmpty && !hasStats && !hasFtp) return null
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {hasFtp && (
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#fef2f2] border border-[#fee2e2]">
            <span className="text-[#94a3b8] font-medium text-[11px] uppercase tracking-wide">
              FTP
            </span>
            <span className="text-[#dc2626] font-bold text-sm tabular-nums">
              {ftp}W
            </span>
          </div>
        )}
